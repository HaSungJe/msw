...
지금: **유닛 정보 팝업 + 캐릭터 선택·위치 이동 + 파티 버프 표시 + 액티브 스킬 잠금 설정 (2026-09-15~16, 화면 v2 위에 추가 — Phase 4 #14·#17, Phase 5 #19·#20, Phase 2 #6을 시범 3직업으로 선행 구현. report: `screen-v2-report.md` Change 260915-1 · 260916-1~8)** · **Next up(2026-09-18): 전투 시험대(`RtsCombatLogic.TestBench`, report 260917-6)에서 히어로 레이징 블로우 연출 확정 → 팔라딘·DK → 나머지 8직업 액티브 1개씩(외형은 260917-1~4로 완료)** → 증강 밸런스(43개 최고점 나이트로드 1위·2위 ×1.3↑, 고점 히어로·DK·신궁) → Phase 2 나머지(스테이지·경제 표) → Phase 3 본편(#15 사거리·타겟, #10 웨이브)

## Scope & Actors
- **플레이어(1~8)** — 자기 구역에서 유닛 영입·배치·레벨업, 증강 선택(전부 팝업). F1~F8로 다른 유저 구역 관전. **탈락 후에는 관전을 계속하거나 방을 나가 새 판을 시작할 수 있다.** **관전(타 유저 구역 보기)은 읽기 전용(2026-09-19)**: 우측 영입 패널이 그 유저 유닛의 직업·레벨 목록으로 바뀌고, 유닛 클릭 → 상세정보 팝업(증강·버프 보기 포함)만 가능. 증강·영입·위치 이동·스킬 잠금/보스전 잠금·레벨업 등 조작은 전부 비활성(서버도 소유자 검증).
- **방장** — 방 생성·난이도 선택·시작.
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 14 | 유닛 엔티티·배치·재배치 | 기본 1기 지급, 트랙 안쪽 배치 슬롯에 세우기, 유닛 비주얼(**MSW 아바타 방식** — 모험가 직업 세트). **위치 이동 및 유닛 간 위치 교환 허용 — 유닛당 3분 쿨다운**(2026-09-15 2분→3분, 대상 변경도 같은 3분). 이동 = 캐릭터 클릭 메뉴 → 발판 틀 강조 → 발판 클릭 | 쿨다운 UI 표기 위치 → **캐릭터 메뉴에 "이동 대기 m:ss"** | 4, 6 | **in-progress**(2026-09-15 시범 3기: RtsUnitLogic·RtsUnitComponent·RtsUnitSelectLogic. 영입 연결·스프라이트 애니는 남음) |
...
| 17 | 유닛 선택·HUD 연동 | ~~초상화·유닛정보 패널~~ → **캐릭터 마우스 오버 손 커서, 클릭 → 우측 작은 메뉴(위치 이동/상세정보)**, 우측 슬롯 클릭 → 상세정보. 부대지정 없음 | — | 2, 14 | todo |
...
| #14 | `RtsUnitLogic:RequestMove` + `RtsUnitSelectLogic`(클라 메뉴·이동 모드) | 이동·교환 명령 | 재배치(유닛당 3분 쿨다운, 서버 검증). 이름은 RtsTower→RtsUnit으로 확정 |
...
| 20 | 레벨 스킬 해금·표시 | 특정 레벨 도달 시 스킬 해금(액티브·패시브). 유닛 팝업에 보유/미해금 스킬 표시, 툴팁. **액티브 스킬 잠금 설정 [사용 / 보스전 잠금 / 잠금]**(2026-09-16 선행 구현 — `RtsUnitComponent.SkillLocks` + `RequestSkillLock` + 팝업 토글, 전투 루프 #15가 읽음) | 액티브 스킬 발동 방식(자동 vs 수동 쿨다운) | 2, 19 | todo |
...
### Phase 6 — 증강 시스템 (판을 가르는 랜덤 축 — **사용자 새 안 작성 중**) · depends: Phase 5, Phase 7
> 기존 안(브론즈~프리즘 4등급, 70/20/9/1, 보스 클리어마다 카드 지급)은 보류. **새 안은 `.info/augmentation.md`에 사용자가 직접 채우는 중(2026-09-16 시작)** — 헤더 규칙: **라운드별로 정해진 등급 지급**, 게임 끝까지 총 **43개**(브론즈 15 / 실버 15 / 골드 10 / 프리즘 3), **프리즘 3개 = 자쿰·루시드·진 힐라**, 스킬 강화 증강은 **스킬 데미지%에 합연산**(레이징 블로우 200% + 20% = 220%). 등급별 풀: 능력치(크리 확률/크리 데미지/공격력 +n/공격력 +%) · 스킬 강화(액티브/패시브) · 골드부터 상태 이상(기절·빙결 + 그 상태의 적에 추가 데미지) · 골드·프리즘 특수 스킬. #21의 "보스 처치 시에만 지급"은 이 안과 맞춰 다시 확정할 것(라운드 스케줄 vs 보스 처치). 유닛 팝업 우측 "증강" 탭(`RtsUnitBuffLogic.GetAugmentsFor` 스텁)이 이 데이터의 표시 자리.

| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 21 | 증강 지급 트리거 | **라운드별로 정해진 등급을 지급**(2026-09-16 사용자 안 — `.info/augmentation.md`). 총 43개(브 15 / 실 15 / 골 10 / 프 3), 프리즘 = 자쿰·루시드·진 힐라 라운드. 지급 조건 없음 — **보스를 못 잡으면 그 자리에서 탈락(게임 오버)** 이라 "처치한 유저만"이라는 분기가 필요 없다. 지급 시 **3택1 팝업**(닫기 없음 — 하나는 받아야 진행) | 지급 라운드표(어느 라운드에 어느 등급) — augmentation.md에 사용자가 채움 | 9, 25 | todo |
| 22 | 증강 결정(서버 권위) | 서버가 그 라운드 등급의 풀에서 **후보 3개**를 뽑아 제시. 선택 1개를 그 유저 보유에 추가. 랜덤 요소는 여기에만. **같은 능력치 증강 중복 허용 — 전부 합연산**(공격력 +3% 두 번 = +6%; 공격력%는 스킬의 공격력%와도 합, 스킬 강화는 스킬 배율에 합) | 후보 3개 안에 같은 증강 중복 허용 여부 | 8, 21 | todo |
...
| 24 | 증강 효과 적용 훅 | 효과가 능력치·데미지·경제에 꽂히는 지점 확보. 능력치(+공격력·공격력%·크리 확률·크리 데미지) → `RtsUnitBuffLogic.ApplyToStat`과 같은 자리, 스킬 강화 → 스킬 배율 합, 상태 이상(기절·빙결 + 그 상태의 적에 최종 데미지 +10%)은 **개념만 — 몬스터 상태 시스템이 생긴 뒤 구현** | 효과 표현 방식(modifier 체인) | 16, 23 | todo |
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 25 | 보스 개체·보스 스테이지 | 60초 보스 스테이지, 보스 전용 HP·패턴(최소). **메소 없음. 제한 시간 안에 보스를 못 잡으면 그 유저는 탈락(게임 오버)**(2026-09-16 확정 — 누적 110마리와 별개의 탈락 조건). 증강은 라운드 스케줄로 지급(#21). **한 스테이지 다중 보스**(아이온+얄다바오트)·**연속 보스 스테이지**(검은마법사 3연전) 지원 | 보스 패턴 범위(단순 고HP vs 행동 패턴) | 7, 11 | todo |
...
| 36 | 스테이지별 BGM·몬스터 스킨 | 스테이지(맵)마다 BGM 1곡 + **몬스터별 기본 자산 3개**(이동 애니메이션 클립·사망 애니메이션 클립·사망 효과음 RUID — 2026-09-14 확정)를 테이블에 연결. 피격음은 몬스터가 아니라 **스킬별 타격음**(#20/#16). 원본 목록은 사용자가 정리 중 | 스테이지 전환 시 BGM 크로스페이드 여부 | 7, 9 | todo |
...
- **이동 속도는 수치(100 기준)** — 속도 100 = 초당 3.5유닛(`RtsConfigLogic.SpeedUnitPerSec`, 2026-09-14 달팽이 시연 속도 기준). 테이블·난이도 배율은 이 수치를 다루고, 유닛/초 환산은 `SpeedToUnitsPerSec` 한 곳에서만.
- **몬스터 = 자산 3개** — 이동 애니메이션 클립·사망 애니메이션 클립·사망 효과음. 피격음은 스킬 쪽(타격음). 몬스터 테이블(#7/#36)의 컬럼도 이 3개.
...
- [x] **과금(2026-09-15 확정)** — 스킨은 기본(직업 세트) / **월드 아바타**(내 코디) 2개, 프리미엄 프리셋 없음. 월드 아바타 = 월드 상점 **아이템 "월드 아바타 30일" 1,600 월드코인**(재구매 = 30일 연장, 만료 시 기본 복귀). 상품 `QK03ZI15E` 비공개 등록, 지급은 `RtsUnitLogic.ProcessPurchase`(DataStorage `avatarUntil`). 결제 연동 확인: Maker 테스트 구매(코인 차감 없음) 창까지 실측, 정산은 판매액 35%
- [x] **유닛 재배치(2026-09-09 확정, 2026-09-15 3분으로)** — **위치 이동·유닛 간 위치 교환 허용, 유닛당 3분 쿨다운(대상 변경도 3분)**. 쿨다운은 서버 권위로 검증한다. (배치 슬롯 30칸은 최대 **6기** 대비 크게 과잉 — 꼬인 경로 맵으로 바꿀 때 슬롯 수도 같이 재설계)
...
- **몹 비주얼 — 해결(2026-09-14)**: 초록달팽이 이동 클립 `e7d919d7b4724c65b2f53164c2873ae0` / 사망 클립 `a25ed762135448ccb2307604f3179565`를 `SpriteRUID`에 넣어 트랙 순환 → 3초 후 사망 전환까지 실측 성공. 예전 실패는 RUID 문제였음. 스케일 2.5에서 줌 30% 화면에 적당한 크기. **몬스터 자산 규칙(2026-09-14)**: 몬스터마다 ① 이동 클립 ② 사망 클립 ③ 사망 효과음 3개가 기본(초록달팽이 = 위 2개 + `sound/mob/0100100/die` `30e5a789e9204111924cafc1bc1ce33f`). 공식 몹 사운드는 `sound/mob/<몹ID>/die·damage`. **피격음은 스킬별 타격음**으로 처리, 몬스터 자산에 넣지 않는다.
