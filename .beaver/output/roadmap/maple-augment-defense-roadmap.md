...
- Phase 7 — 보스·클리어·결과 · 유닛 4개 · 의존: Phase 6 · 푼다: Phase 5, Phase 8 · 파일 집합: RtsBossLogic, RtsRunResultLogic, RtsStageLogic, RtsPopupLogic(결과) · **done**(2026-09-24)
...
### Phase 12 — 솔로 출시 (혼자 하는 버전으로 먼저 공개) · depends: Phase 9 · 2026-09-24 사용자 "일단은 솔로 모드로 출시, 그리고 멀티로 업데이트"
...
| 32 | 멀티 동기화·권위 재점검 | 8구역 동시 웨이브·연출 부하, 방 전역/유저별 상태 분리, 서버 권위 전수 점검, 9번째 입장 처리 | — | 31 | todo |
| 42 | 로비 전광판 | 로비에 #44 순위 Top 10(스크롤 100위), 내 순위 하단 고정, 칸 = 아이콘(#40)·닉네임·클리어 횟수. 엔진 리더보드는 ReleaseOnly라 Maker에선 로컬 SortableDataStorage 경로 | 시즌(무한 vs 월간 리셋) | 44, 31 | todo |

### Phase 10 — 아이콘·테마·플레이어 목록 (꾸미기) · depends: Phase 12
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 35 | 유닛 비주얼 | 11직업 아바타 세트 | — | — | done(Phase 2 #6) |
| 37 | 스테이지별 BGM·보스 비주얼 | 라운드별 BGM(`RtsStageLogic.RefreshBgm`) + 보스 17종 그림 | — | 22, 27 | done(2026-09-23) |
| 40 | 아이콘 컬렉션 | 유저 아이콘 = 몬스터 얼굴, 기본 초록달팽이. 처치 시 **일반 1% · 보스 5%** 획득(중복 무효) → 계정 DataStorage 컬렉션, 컬렉션 화면(격자·미획득 실루엣) | 획득 판정 주체(막타 vs 구역 주인) · 관전 중 획득 · 아이콘 자산(몬스터 1프레임 vs 얼굴 자산) | 22, 27 | todo |
| 38 | 테마 프리셋 확장 | 헤네시스 외 시간의 신전·소멸의 여로·레헬른 등(트랙·타일·장식·BGM), **유료**(헤네시스 기본), 구매·선택은 계정 저장 | 적용 범위(내 구역 vs 방 전체) · 가격 | — | todo |
| 41 | 좌측 플레이어 목록 확장 | 카드 = 원형 아이콘 + 테마 이름 + 닉네임 + **내 카드에만 수정 버튼**(아이콘·테마 선택 팝업), 카드 배경 = 그 유저 테마, 8명 세로 배치·관전 클릭 유지(스케치 `.info/artifacts/player-slot-sketch-260922.png`) | 카드 크기 · 테마 카드용 썸네일 | 38, 40 | todo |
| 39 | 아바타 프리셋(영구 이용권) | 월드 아바타 30일 → 영구 이용권, 직업별 아바타 프리셋 저장 | 슬롯 수 · 저장 단위 | 35 | in-progress(2026-09-24 영구 이용권 전환 — 코드·유닛 창 문구·썸네일·상점 상품 이름/설명 갱신, 종류는 아이템 유지. 직업별 프리셋은 후순위) |

### Phase 11 — 디자인 인계(ChatGPT) · depends: Phase 8, Phase 10(어느 정도 마무리 뒤)
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 45 (신규, 2026-09-24) | 화면 UI 요소 명세 | 모든 화면 요소(엔티티 이름·만드는 곳·위치·크기·색 토큰·글꼴·보이는 조건·동작·효과음)와 디자인 작업 규칙을 `docs/ui-screens.md`로 — ChatGPT가 이것만 보고 이어받게 | — | — | in-progress(2026-09-24 초안) |
| 36 | HUD·화면 디자인 패스 | 프레임 톤·색 토큰·아이콘 아트·레이아웃 다듬기(ChatGPT 담당, `AGENT.md` 규칙 준수) | 인계 시점 · 아트 자산 범위 | 45 | todo |
...
**Progress**: 34/46 units done (8/12 phases; Phase 10은 2/6, Phase 11은 0/2 — #45 명세 초안 완료·검토 대기)
**Next up**: **Phase 12 — 솔로 출시** — 사용자가 솔로로 먼저 출시하기로 했다(2026-09-24). 순위 교체(#44) → 솔로 정리(#46) → 출시 점검(#47), 공개는 사용자가 Maker에서. 그다음 Phase 8 멀티 업데이트 · Phase 10 꾸미기 · Phase 11 디자인 인계.
...
- [ ] **순위 시즌** — 무한 누적 vs 월간 리셋. (Phase 8 #42 전)
- [ ] **아이콘 획득 판정 주체·자산 형태** (Phase 10 #40 전), **테마 적용 범위·가격** (Phase 10 #38 전)
...
## Notes / Risks
- **Play 확인 대기(2026-09-24)**: 효과음, 증강 도감 펼치기, 관전 시 난이도 막대 비노출, 여러 명 투표 패널, 몬스터 초상 정중앙 — 다음 Maker 실행 때 build 로그부터.
