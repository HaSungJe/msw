...
## Direction
최대 8인이 한 방에서 **같은 시간표로 같은 스테이지를 함께 진행**하되, 각자 자기 구역에서 자기 유닛으로 막는다. **승패 개념은 없다** — 결과는 각자 검은 마법사를 깼느냐, 중간에 죽었느냐뿐. 자기 구역의 그리드 트랙(16×13) 발판에 직업 유닛을 세우고, 경로를 따라 도는 몹 웨이브를 막는다. 유닛은 **메소로 영입해 레벨(1~50)을 올린다**(4차 직업 고정, 전직 없음, **동일 직업 중복 영입 불가 — 슬롯 6개 = 서로 다른 직업 6개**). 직업은 모험가 10 + 프리즘 증강으로 해금되는 **영웅 팬텀** = 11. 레벨이 오르면 능력치가 오르고 스킬이 해금된다(각 직업 1·10·20·30·40·50). 처리하지 못한 몹이 **누적 110마리**가 되거나 **보스를 60초 안에 못 잡으면** 내 유닛 전멸 + 비석과 함께 개인 탈락한다. 판을 가르는 랜덤 요소는 **증강**(한 판 32개: 브론즈 10·실버 10·골드 7·프리즘 5 — 라운드별 등급 지급, 3택1; 2026-09-22 20개에서 확대)이다. 속성·내성 같은 상성 체계는 **없다** — 데미지는 damage.md 8단계 수식 그대로. 판단 축은 어떤 직업 조합을 세우고 어디에 레벨을 투자하느냐 + 증강 운. 메소는 **몹을 잡을 때만** 들어오고(총 768,000 — 진 힐라 직전 전원 50레벨, 보스 라운드마다 체크포인트; 시간 수입·보스 메소 없음 — 2026-09-23 사용자, 옛 30만 폐기). 영입은 **5라운드마다 1칸**(자쿰 전 6명). 아케인리버부터 몬스터 체력 2배. 특수 행동(영입·레벨업·증강)은 **화면 옆 버튼 또는 유닛 클릭 → 팝업**으로 처리해 플레이 화면을 넓게 쓴다. **난이도는 의도적으로 매우 높게** 잡아, 검은 마법사 격파 자체가 도전 과제가 되도록 한다. (방향은 느슨하게 유지 — 언제든 바뀔 수 있음)
...
flowchart LR
  ...
  P3 --> P6["Phase 6 · 스테이지·몬스터 웨이브 (5)"]:::inprogress
  P6 --> P7["Phase 7 · 보스·클리어·결과 (4)"]:::inprogress
  P7 --> P5["Phase 5 · 유닛 관리 — 영입·레벨 (3)"]:::todo
  P5 --> P4["Phase 4 · 증강·프리즘 (6)"]:::inprogress
...
- Phase 4 — 증강·프리즘 · 유닛 6개 · 의존: Phase 5(영입·레벨이 있어야 대상 지정·밸런스가 실전값; 지급 라운드 = Phase 6·7 이벤트) · 푼다: Phase 8 · 파일 집합: RtsAugmentTableLogic(신규), RtsAugmentLogic(신규), RtsAugmentPopupLogic(신규), RtsUnitBuffLogic(효과·증강 탭), RtsJobTableLogic(프리즘 증강 11종 → 스킬 12종), RtsSkillFxLogic(프리즘 연출), RtsHudLogic(증강 버튼) · in-progress(#16 프리즘 효과·연출 done 2026-09-22; #14·#15·#17·#18은 `/beaver:direct`부터 — 규칙은 `.beaver/memory/balance.md` '증강 3택1·대상 지정 규칙' + `.info/augmentation.md`)
...
- Phase 6 — 스테이지·몬스터 웨이브 · 유닛 5개 · 의존: Phase 3(영입은 개발모드 무료 영입으로 대신) · 푼다: Phase 7, Phase 10 · 파일 집합: RtsStageTableLogic(신규 — 몬스터 RUID 매칭 포함), RtsStageLogic(신규), RtsWaveLogic(신규), RtsMonsterComponent, RtsTrackWalkerComponent, RtsAugmentLogic(라운드 트리거 연결), RtsCombatLogic(시험대 제거), RtsDemoLogic(제거), RtsHudLogic(시간 칩) · in-progress(2026-09-23 Phase 7과 한 plan)
- Phase 7 — 보스·클리어·결과 · 유닛 4개 · 의존: Phase 6 · 푼다: Phase 8, Phase 10 · 파일 집합: RtsBossComponent(신규), RtsBossLogic(신규), RtsRunResultLogic(신규), RtsStageLogic(보스 스테이지) · in-progress(2026-09-23 Phase 6과 한 plan)
...
지금: **Phase 6 + Phase 7 한 plan 진행 중(2026-09-23 — 스테이지·웨이브·보스·클리어·결과, `.beaver/output/plan/rts-stage/stage-wave-boss-plan.md`)** · 다음: Phase 5 유닛 관리 → Phase 4 증강 → Phase 8 방·멀티(+로비 전광판) → Phase 10 테마·아바타·아이콘 프리셋 → Phase 9 밸런싱 · Next up: `/beaver:build stage-wave-boss`
...
| 43 (신규, 2026-09-23) | 증강 구매 | 메소로 증강을 산다 — 레벨업을 다소 포기하더라도 증강에 투자하는 선택지. 가격 **3,000부터, 살 때마다 +800**(3,000 → 3,800 → 4,600 …, 2026-09-23 사용자 — 1,000 고정 → 5,000 + 1,000 → 3,000 + 800). 등급 확률 **브론즈 80 / 실버 15 / 골드 4.5 / 프리즘 0.5%**(사용자, 합 100 — 처음 85%는 합 105라 80%로 확정) → 그 등급 풀에서 굴림. 서버 권위(메소 차감·굴림), HUD/증강 창에 구매 버튼 | (2026-09-23 build: 등급 3택1 선택지로 지급 · 판마다 3,000 초기화 · 상한 없음 · 고르지 않은 선택지 3개면 못 뽑음 · 프리즘 다 가졌으면 골드) | 15, 17 | **in-progress**(Phase 6+7 build에 합류 — RtsAugmentLogic.RequestBuy, HUD '증강 뽑기') |
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 22 (옛 7·36 일부) | 스테이지·몬스터 편성 테이블 + **몬스터 RUID 매칭** | `.info/monster-wave.md`의 **132 스테이지**(몬스터 115 + 보스 17)를 테이블화. 월드지역>지역>맵>몹 계층, 종류 3종, 몹 풀 참조형([73] 59~65 균등), 다중 보스. 몹마다 HP·드랍 메소·**자산 3개 RUID**(이동 클립·사망 클립·사망 효과음 — 공식 라이브러리에서 매칭) — 속도는 전부 동일 | 쉬는 스테이지 지속 시간 · 몹 HP 첫 값(임시) | — | in-progress |
| 23 (옛 9) | 스테이지 진행·타이머(**방 전역**) | 방 하나에 타이머 하나 — 전원 같은 스테이지 번호로 시간순 진행. 몬스터 20초 / 보스 60초 / 쉬는. 시간 칩에 스테이지 번호·지역명·남은 시간, 전환 이벤트 발행 → **증강 라운드 지급(#15)·BGM이 여기에 붙음** | 시작 시점(전원 준비 vs 방장 시작) · 준비시간 유무 | 22 | in-progress |
| 24 (옛 10·12) | 몹 스폰·트랙 순환(**구역별**) | 스테이지 시작 시 살아 있는 구역마다 초당 2마리(40) 스폰 → 이동 클립으로 등속 순환(`RtsTrackWalkerComponent` 재사용). 시연 몬스터 제거, 시험대(주니어 발록·테스트 프리즘)는 개발모드 토글로 남긴다 | — | 4, 23 | in-progress |
| 25 (옛 11 + 경제 배분) | 처치·드랍·정리 | HP 0 → 사망 클립·사망음 → 제거, **드랍 100% 지급** — 총 30만을 지역별 분포(아케인리버 이전 20% … 리멘 20%)로 스테이지별 드랍에 배분·검산, 시체는 대상에서 제외 | 사망 클립 길이만큼 대기 | 24 | in-progress |
| 26 (옛 13) | 누적·탈락 판정 | 처리 못 한 몹 누적 카운트 표시, **110 도달 시** 내 구역 유닛 전멸 + 비석 + 관전 전환(F1~F8 유지) 또는 방 나가기 | 탈락 직후 선택 UI 형태 | 25 | in-progress |
...
### Phase 7 — 보스·클리어·결과 (한 런의 끝) · depends: Phase 6 · **2026-09-23 사용자 "보스까지 다 만들자" → Phase 6과 한 plan**(`.beaver/output/plan/rts-stage/stage-wave-boss-plan.md`)
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 27 (옛 25) | 보스 개체·보스 스테이지 | 60초, 보스 HP·긴 체력바(2.6), 메소 없음, **못 잡으면 탈락**. 다중 보스(아이온+얄다바오트)·연속 3연전 | 보스 패턴 범위(고HP만 vs 행동) — 이번엔 고HP·트랙 이동만 | 22, 23 | in-progress |
| 28 (옛 26) | 보스 등장·타격(구역 내) | 보스 스테이지가 되면 **일반 몹 보관(숨김·정지, 끝나면 제자리 복귀)** + 각 구역 S에서 보스 출발 → **60초에 트랙 한 바퀴**(감속·기절 무시), E 도달 = 탈락. 보스전 **전 유닛 사거리 10**(`.info/memo.md` 사용자 규칙 — 옛 '구역 중앙 보스' 대체), 보스전 잠금 스킬 제외 | — | 9, 27 | in-progress |
| 29 (옛 27) | 클리어·탈락 판정(유저별) | 검은 마법사 3연전 격파 = 클리어, 누적 110 또는 보스 시간 초과 = 탈락. 방은 마지막 스테이지 종료 또는 전원 탈락/퇴장까지 유지 | — | 26, 28 | in-progress |
| 30 (옛 28) | 결과 화면·기록 | 도달 스테이지·처치 수·클리어 여부(유저별). 런이 끝나면 **최종 등수용 점수 등록**: `점수 = 최대 클리어 라운드 × 100,000 + 탈락한 보스전에서 깎은 보스 HP 비율(0~10,000 = 0.00~100.00%)` → 개인 최고만 갱신(`LeaderboardScoringStrategy.High`), extras에 닉네임·클리어 여부·날짜 | 기록 = UserDataStorage 개인 최고(2026-09-23), 결과 팝업 + 나가기 · 클리어자끼리의 2차 기준(클리어 시간?)은 Phase 8 #42 | 29 | in-progress |
...
**Progress**: 14/43 units done (3/10 phases; Phase 10은 1/7)
**Next up**: **Phase 6 + 7 build** — plan 승인 뒤 `/beaver:build stage-wave-boss`(자산 매칭 → 표 생성 → 진행·웨이브·보스·결과). 그다음 Phase 5 유닛 관리 → Phase 4 증강(#14·#15·#17·#18) → Phase 8 방·멀티(+로비 전광판 Top 10~100) → Phase 10 테마·아바타·아이콘 프리셋(+좌측 플레이어 목록 확장) → Phase 9 밸런싱.
...
- [x] **쉬는 스테이지 지속 시간** — 30초(2026-09-23 사용자). 시작 = 입장 5초 뒤 자동(방 기능 뒤에도 같음), 누적 = 필드에 살아 있는 몹 수, 영입 = 항상 Lv1(영입가는 Phase 5), 체력 = 예산 역산 + 자쿰·루시드·진 힐라 뒤 계단 상승(`.info/monster.md` 끝 표)
...
- [x] **경제 원칙** — 처치 시에만. **2026-09-23: 총 768,000, 메소 오름세·아케인 ×1.67·스페셜(보스 앞 라운드) ×2·10메소 단위, 레벨업 비용표 교체(`.info/level.md` 5차), 영입은 5라운드마다 1칸·메소 안 씀** → Phase 5 #19 영입가 표는 재검토
- [x] **증강** — 32개(10/10/7/5, 2026-09-22), 브~골 능력치 I/II/III + 확률, 프리즘 5 = 자쿰·시그너스·스우·루시드·진 힐라(2026-09-23 핑크빈 → 스우; 팬텀 영입 + 직업 전용 스킬 12종, 재출현 없음), 전부 합연산. **브~골 27개 라운드 지급표 = augmentation.md**(몬스터 스테이지 등간격 4.2칸, 브·실·골 비례 혼합 — 2026-09-22 사용자 "골고루")
- [ ] **후보 3개 규칙** — 같은 증강 중복 노출 허용 여부, 미보유 직업 전용 프리즘 노출 여부. (Phase 4 #15 전)
- [x] **증강 구매 흐름**(2026-09-23 build 기본값: 3택1 · 판마다 초기화 · 상한 없음 · 대기 3개면 불가 · 프리즘 소진 시 골드) — 확률은 확정(브론즈 80·실버 15·골드 4.5·프리즘 0.5, 2026-09-23). 가격도 확정(5,000부터 살 때마다 +1,000). 남은 것: 산 증강을 3택1 선택지로 줄지 바로 1개 지급할지, 구매 횟수 상한, 프리즘 당첨 시 보유 프리즘·팬텀 처리, 메소 경제 영향. (Phase 4 #43 전)
- [x] **보스전 사거리** — 전 유닛 사거리 10 고정, 보스는 트랙을 60초에 한 바퀴(`.info/memo.md` 사용자 규칙, 2026-09-23 plan에 반영)
