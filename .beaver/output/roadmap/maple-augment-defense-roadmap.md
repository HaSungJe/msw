...
> **v260924-1 (2026-09-24) 솔로 출시** — 게임 버전 = `v` + YYMMDD + `-` + 그날 커밋 순번(사용자 "이제부터 버전에 날짜 + 커밋 횟수가 붙어, 이게 게임 버전"). 출시 노트의 '개발 예정 로드맵' 1~5 = Phase 8 · 13 · 10 · 14 · 11.
> **2026-09-25 순서 변경(사용자)**: ① 개인 테마 프리셋(Phase 10) + 아이콘 프리셋(Phase 14) 동시 → ② 전체 UI 개선(Phase 11) → ③ 멀티플레이(Phase 8) → ④ 순위표(Phase 13). 현재 버전 **v260926-4**.
...
flowchart LR
  ...
  P12 --> P14["① Phase 14 · 아이콘 프리셋 (1)"]:::inprogress
  P10 -. 동시 가능 .- P14
  P10 --> P11["② Phase 11 · 전체 UI 개선 (3)"]:::inprogress
  P14 --> P11
  P11 --> P8["③ Phase 8 · 멀티플레이 (2)"]:::todo
...
- ② Phase 11 — 전체적인 UI 개선 · 유닛 3개 · 의존: Phase 10, 14 · 푼다: Phase 8 · 파일 집합: RtsHudLogic(좌측 플레이어 카드·HUD), RtsPopupLogic·RtsUnitPopupLogic·RtsDifficultyLogic·RtsMonsterInfoLogic(Client 메서드), `docs/ui-screens.md`, `AGENT.md` · in-progress(#45 UI 명세 done)
- ③ Phase 8 — 멀티플레이(대기방·매칭·방 생성/참가) · 유닛 2개 · 의존: Phase 11(2026-09-25 순서) · 푼다: Phase 13 · 파일 집합: RtsRoomLogic(신규 — 대기방·매칭·방), RtsBootstrapLogic, RtsStageLogic(방 인원·레디), RtsPopupLogic(대기방 화면) · todo
- ④ Phase 13 — 순위표 · 유닛 1개 · 의존: Phase 8(대기방에 붙음) · 푼다: — · 파일 집합: RtsRunResultLogic(순위 조회), RtsLeaderboardLogic(신규 — 대기방 순위표), RtsPopupLogic(순위 창) · todo

동시 가능: Phase 10 · Phase 14 (테마 = RtsThemeLogic·RtsZoneLogic, 아이콘 = 새 RtsIconCollectionLogic·처치 훅 — 둘 다 RtsPopupLogic에 팝업을 하나씩 더하므로 그 파일만 순서 조율). 그 뒤 Phase 11 → 8 → 13은 순서대로.

지금: **v260926-4**(v260924-1 솔로 출시 뒤 패치) · Next up: **① Phase 10 개인 테마 프리셋 + Phase 14 아이콘 프리셋(동시)**

### 출시 노트 v260924-1 — 개발 예정 로드맵 (사용자 원문 — 2026-09-25 진행 순서는 3·4 → 5 → 1 → 2로 바뀜)
1. 멀티플레이(매칭/방 생성 및 참가) → Phase 8
2. 순위표 (어려움/매우어려움/극악 난이도의 검은마법사 클리어 횟수) → Phase 13 (게임 안 '순위' 버튼·결과 창 순위 줄은 2026-09-25 사용자 요청으로 제거 — 순위는 대기방에서만)
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 44 (신규, 2026-09-24) | 순위 기준 교체 | **어려움·매우어려움·극악 난이도별 탭 3개, 각 탭 = 그 난이도에서 검은 마법사 4페이즈를 깬 횟수 순**(옛 최종 메소·최대 라운드 기준 폐기). 클리어마다 서버가 횟수 +1(테스트 모드·보통 이하 제외), 결과 창에 순위·횟수, 왼쪽 아래 '순위' 창 — **2026-09-25 게임 안 표시는 제거**(사용자 "지금 저기서 보일 게 아니야"), 기록 저장만 유지 | 동점 = 그 횟수에 먼저 도달한 사람이 위 | 30 | done(2026-09-24 a9dcef9) |
...
### ① Phase 10 — 유저 개인 테마(트랙+배경) 프리셋 · depends: Phase 12 · 출시 노트 3 · Phase 14와 동시
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
...
| 38 | 개인 테마 프리셋 | 유저마다 자기 구역의 **트랙 + 배경** 테마를 고른다 — 헤네시스 외 시간의 신전·소멸의 여로·레헬른 등 프리셋(트랙·타일·장식), 선택은 계정 저장 | 유료 여부·가격 · 테마 목록 · 적용 범위(내 구역만) | — | in-progress(2026-09-25 plan `rts-custom/theme-icon-preset-plan.md` — 헤네시스·엘리니아·페리온·커닝시티·리스항구 5종 무료, 뼈대 + 임시 모양, 디자인은 ChatGPT) · build 완료 — report `rts-custom/theme-icon-preset-report.md` |

### ① Phase 14 — 유저 아이콘 프리셋 · depends: Phase 12 · 출시 노트 4 · Phase 10과 동시
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 40 | 아이콘 프리셋·컬렉션 | 유저 아이콘 = 몬스터 얼굴, 기본 초록달팽이. 처치 시 **일반 1% · 보스 5%** 획득(중복 무효) → 계정 DataStorage 컬렉션, 컬렉션 화면(격자·미획득 실루엣)에서 골라 내 아이콘으로 | 획득 판정 주체(막타 vs 구역 주인) · 아이콘 자산(몬스터 1프레임 vs 얼굴 자산) | 22, 27 | in-progress(2026-09-25 같은 plan — 획득 = 매우어려움 1% · 극악 2%(처치 시, 보스 포함), 획득 일시 저장) · build 완료 — report `rts-custom/theme-icon-preset-report.md` |

### ② Phase 11 — 전체적인 UI 개선 · depends: Phase 10, 14 · 출시 노트 5
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
...
| 41 | 좌측 플레이어 목록 카드 | 카드 = 원형 아이콘(#40) + 테마 이름(#38) + 닉네임 + **내 카드에만 수정 버튼**(아이콘·테마 선택 팝업), 카드 배경 = 그 유저 테마, 8명 세로 배치·관전 클릭 유지(스케치 `.info/artifacts/player-slot-sketch-260922.png`) | 카드 크기 · 테마 카드용 썸네일 | 38, 40 | in-progress(2026-09-25 같은 plan으로 당겨옴 — 배경 = 테마 썸네일, 내 카드 초록 테두리 + [수정]) · build 완료 — report `rts-custom/theme-icon-preset-report.md` |
| 36 | 화면 디자인 패스 | 프레임 톤·색 토큰·아이콘 아트·레이아웃 다듬기(ChatGPT 담당 가능, `AGENT.md`·`docs/ui-screens.md` 규칙 준수) | 사용자 최종 시각 확인 | 45 | in-progress(백색 물결 UI·등급/스킬 합성 아이콘·6축 능력치·필터 개선 적용, ui-renewal-report 참조) |

### ③ Phase 8 — 멀티플레이(대기방·매칭·방 생성/참가) · depends: Phase 11 · 출시 노트 1
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 31 | 대기방·매칭·방 | **대기방 → 솔로 / 매칭 / 방 만들기 / 방 목록**(2026-09-24 사용자). 2026-09-25 메모: 방은 인스턴스 룸(RoomService — 월드 최대 인원만 올리면 빈자리에 남이 낀다), 매칭·방 판은 `RtsStageLogic.SoloMode = false`(일시정지 = 솔로 모드 전용): ① 솔로 = 바로 시작 ② 방 만들기 = 방 제목 + 비밀번호(선택 — 넣으면 비공개 방) → 만든 뒤 대기자 확인 → **모두 레디하면 방장이 시작** ③ 방 목록 = 목록에서 골라 입장, 비공개 방은 비밀번호 입력, **방 제목 검색** ④ 매칭 = 랜덤 유저끼리 묶어 바로 시작. 게임은 인스턴스 방(1~8인)에서, 결과 창 '나가기' = 대기방으로. 출시 설정 최대 인원을 다시 늘린다 | 매칭 인원(몇 명 모이면 시작 · 대기 시간 상한) · 방 목록 갱신 주기 · 진행 중 입장 처리(관전 대기 → 다음 판) | — | todo |
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 42 | 순위표(대기방) | 대기방에 난이도별 순위표(#44와 같은 기준 — 어려움·매우어려움·극악 검은 마법사 4페이즈 클리어 횟수) Top 10(스크롤 100위), 내 순위 하단 고정, 칸 = 아이콘(#40)·닉네임·클리어 횟수. 게임 안 순위 표시는 2026-09-25 제거 — 순위 창 코드(RtsPopupLogic.BuildRank)를 대기방에서 다시 씀 | 시즌(무한 vs 월간 리셋)  | 44, 31 | todo |
...
| 48 (신규, 2026-09-24) | 배속 모드(2배) | **모든 속도 2배**, **방 단위**(방 만들기에서 선택, **매칭도 배속을 골라서** 같은 배속끼리 — 사용자 "진짜 나중에"). 구현 메모: 서버 판정 시각(`ServerElapsedSeconds` ~30곳)을 배속으로 흐르는 게임 시계 하나로 모으고, 타이머 지연(~80곳, 대부분 스킬 연출)은 배속으로 나누며, 클라는 `_UtilLogic:SetClientTimeScale(2)`(클라 전용 — 애니·연출, 타이머에도 먹는지는 Maker 실측 필요). 정할 것: 순위 합산/분리 · 배속 단계(2배만?) | todo(Phase 8 뒤) |
...
**Progress**: 38/47 units done (9/14 phases) — 현재 버전 **v260926-4**(솔로 출시 v260924-1 뒤 패치)
**Next up**: **① Phase 10 개인 테마 프리셋 + Phase 14 아이콘 프리셋(동시)** — 2026-09-25 사용자 순서. 좌측 플레이어 카드(Phase 11 #41)가 아이콘·테마를 쓰므로 먼저. 시작 전 정할 것: 테마 유료 여부·가격·목록(#38), 아이콘 획득 판정 주체·자산 형태(#40). 그다음 ② 전체 UI 개선 → ③ 멀티플레이 → ④ 순위표.
