...
> **2026-09-25 순서 변경(사용자)**: ① 개인 테마 프리셋(Phase 10) + 아이콘 프리셋(Phase 14) 동시 → ② 전체 UI 개선(Phase 11) → ③ 멀티플레이(Phase 8) → ④ 순위표(Phase 13). 현재 버전 **v260925-13**.
...
지금: **v260925-13**(v260924-1 솔로 출시 뒤 패치) · Next up: **① Phase 10 개인 테마 프리셋 + Phase 14 아이콘 프리셋(동시)**
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 40 | 아이콘 프리셋·컬렉션 | 유저 아이콘 = 몬스터 얼굴, 기본 초록달팽이. 처치 시 **일반 1% · 보스 5%** 획득(중복 무효) → 계정 DataStorage 컬렉션, 컬렉션 화면(격자·미획득 실루엣)에서 골라 내 아이콘으로 | 획득 판정 주체(막타 vs 구역 주인) · 아이콘 자산(몬스터 1프레임 vs 얼굴 자산) | 22, 27 | in-progress(2026-09-25 같은 plan — 획득 = 매우어려움에서만 처치 시 1%(보스 포함), 획득 일시 저장) · build 완료 — report `rts-custom/theme-icon-preset-report.md` |
...
**Progress**: 38/47 units done (9/14 phases) — 현재 버전 **v260925-13**(솔로 출시 v260924-1 뒤 패치)
