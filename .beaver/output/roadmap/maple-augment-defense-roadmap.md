...
- 한 판 = 스테이지를 스토리 순서로 나열한 것. 마지막은 검은 마법사 **3연전** 보스 스테이지.
...
| 6 | 직업 10종 테이블 | 전사3·마법사3·궁수2·도적2. **1차로 시작 → 2·3·4차는 레벨+메소, 5차는 해금 조건(아카이럼·매그너스 격파) 충족 후**. 기본 스탯·전직별 스킬 목록(**전직당 1개 이상, 패시브 포함**). **속성은 스킬 단위**로 부여(아래 매핑표) | 5차 해금 플래그 위치(매치 전역 vs 유저별) | 5 | todo |
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 9 | 스테이지 진행·타이머 | 몬스터 20초 / 보스 60초 / 쉬는 스테이지 진행, 스테이지 번호·지역명·남은 시간 HUD 표시, 다음 스테이지 전환 | 스테이지 간 준비시간 유무 | 7 | todo |
...
| # | Unit | Goal | Key decision | Depends | Status |
|---|------|------|--------------|---------|--------|
| 29 | 방 시스템 | 방 생성/입장/시작(1~8인), 세션 격리. **탈락자는 방을 나가 새 방을 만들거나 다른 방에 들어갈 수 있음**(진행 중인 방은 그대로 유지) | 인스턴스 맵 구조 | 27 | todo |
...
- **탈락자 대기 시간 — 해소(2026-09-13)**: 탈락자는 **관전을 계속하거나 방을 나가 새 방을 만들어 다시 플레이**할 수 있다. 완주 약 57분은 검은 마법사까지 간 생존자만 겪는 길이이고, 탈락자가 그걸 기다릴 필요는 없다. 구현은 #13(탈락 직후 선택)과 #29(방 나가기·재생성)에서.
