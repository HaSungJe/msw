...
- AGENTS.md application: not needed(non-code — 데이터 규칙)
...
- Rule: 원문은 `.info/character.md` 각 스킬(RUID·경로·크기·시간·플래그), 코드는 `RtsJobTableLogic.GetSkillsRaw("il"/"fp"/"bishop")`. 요약 — **썬콜** 체인 라이트닝 = 2221006 effect fa693d02(시전)·ball/0 fde245ae(빔 체인 조각 50×36)·use/hit 사운드, 타격은 **썬더볼트 2201005 hit/0 33fc60d9**(하늘에서 내리치는 번개, 사용자 "썬콜은 영…"에 교체); 블리자드 = **skill/80003323 세트(사용자 지정 2026-09-18)**: 등 뒤 effect 96b26165(땅에서 솟는 얼음 결정 군집, 3.0초, 원점 바닥 중앙, model 형식) + 대상마다 hit/0 735ad75a(하늘 고리 → 얼음 낙하 → 발에서 산산조각, 착지 1.68초, 원점 착지점) — tile/0~8은 같은 낙하 얼음의 높이·시각 변형(미사용), 세트 사운드는 없음 → 2221007 use/hit 유지. 빙결 표시 = **결정체 아이콘 2221012/effect 223e1ac7**(18프레임: 0~12 형성 → 12 완성 결정체 유지 → 13~17 부서짐; 몬스터 정중앙 체력바 위, 세계 0.7 — 2026-09-20 사용자 "새 아이콘이 더 낫다 → 기존 것 대체", "정중앙 머리 위·더 작게"). 이전: 눈꽃 21001008/mob 21628ca1(09-18), 블리자드 아이콘 a05d1321. 몬스터 타격 클립 = **2221007/hit/0 99a29e16**(파란 얼음 폭발 100×128, 6프레임 — 2026-09-20 "타격 순간 몬스터 위 이펙트"). 다른 얼음 hit 후보(미사용): 2221012/hit/0 818738ee(큰 파란 별 폭발 204×180), 2211002/hit/0 c821bdbd(아이스 스트라이크 흰 폭발), 2201008/hit/0 9bdeae82(콜드빔 얼음 파편). 순서: 군집 3.0초, 낙하 0.5초 시작 → 착지·판정 2.2초 → 조각 잔해 3.0초 → 군집 끝(사용자 "캐릭터 이펙트 종료 직전 → 타격 → 타격 이펙트 끝 → 캐릭터 이펙트 종료"), 후딜 3.0. 타격 얼음 스케일은 0.7→0.95→1.5→**2.0**으로 키워 확정(작게 보임 — 원작 px 크기에 비해 이 게임 줌에선 2배쯤이 맞음). 거친 후보: 2221007 effect(눈빛 기둥, 등 뒤 → 거절)·effect0(발밑 결정, 작음)·tile/4(낙하 얼음 옛 판), 2221012 effect(결정 감싸기), 아이스 스트라이크 2211002 기둥·hit, 프리징 브레스 2221011, 2221052 회전 구슬, 엘퀴네스 2221005 attack, 썬더 스피어 2211010. 사용자가 Maker 피커에서 직접 고른 RUID는 metadata_bulk로 정체(skill 경로)를 확인하고 `*<스킬ID>`로 세트 전체(effect/hit/tile)를 훑는다. 옛 블리자드 2221003·옛 레이징 블로우처럼 **빅뱅 이전 4차 클립은 라이브러리에 없음(사운드만)**. **불독**(사용자 OK 2026-09-18) 포이즌 리전 = 3차 포이즌 미스트 2111003 effect b0d47a89 + tile/0~8 독안개 9종(트랙 65칸 루프) + mob e42be518(틱) + use/hit(원작 2121055는 사운드만); 도트 퍼니셔 = 2121052 effect fce52ddb(시전 불꽃 해골) + 사운드 5차 400021001 use/use2(1.18초; 이 ID엔 클립 없음) + 구체 = **자체 업로드 RtsDotOrb01~03**(스킬 아이콘 d998c591·20a64945를 원형 컷, 3769c672·b0b300d3·da5ad791, 1.8배 맥동·240°/초 자전) + 박치기 폭발 12121055 hit/0 9b8b4484(0.7배). 12121055 effect 50781432의 0~4 프레임은 납작한 '불접시', 5~11은 둥근 화염구(폐기). 흐름: 시전음 동안 동심원 고리(2.0/+1.2, 둘레÷1.2개)에 30개 생성·호버 → 시전음 끝에 가장 가까운 적(보스 우선) 중앙으로 4.4유닛/초 → 박치기 판정. **비숍** 엔젤레이 = 2321007 effect e86eef7f(등 뒤 금빛 날개)·ball 77bc39d2(빛살 281px, 촉 왼쪽)·hit/0 341a2fd0·hit 사운드(use 없음); 제네시스 = 2321008 effect0 1c434f7c(천사, 원점 왼쪽 치우침 → assetFacesLeft=false로 항상 등 뒤)·tile/0~5 빛기둥(착지 ≈1.1초)·hit/0 74f50015·use/hit. 빙결 아이콘 = 2221007 icon a05d1321. 아바타 액션에 magic 계열은 없음 → 마법사는 swingO1/O2/O3(한손) + 대기 stand1.
- Scope: project
- Rationale: User request 2026-09-18 "마법사 3 → 도적 2 → 팬텀, 그 뒤 실제 라운드 테스트"
- AGENTS.md application: not needed
- Priority: takes precedence over defaults

## 나이트로드 쿼드러플 스로우·쉐도우 파트너 자산(2026-09-20 확정)
- Rule: 쿼드러플 스로우 = 원작 4121013 — 시전 클립 effect `ff9ced53e871474c9a2ae167e0c75ef1`(던지는 손 궤적, 왼쪽 보기, 스케일 1.4) + 투사체 **뇌전수리검 02070011 bullet `99a112c1dcbb4cf69fccfca53fbb6620`**(사용자 지정; 일비 02070006 `f302967686104c0180e24d45377a08bb`은 대안) 4발 0.1초 간격, 배 앞(projDx 0.35·projDy 0.55)에서 출발 + 타격 hit/0 `6d26a151289342db913d50bf9f84a96b` + use `747c5e5435b942a494f94032dabbffb2` / hit `4709f06b2fb543f7a980fdbbb23441b1`. 모션 **swingO1 + hideWeapon**(shootF는 활 자세). 사거리 2. 쉐도우 파트너(30~) = 원작 skill/4111002/special 검은 실루엣 클립 33종(stand1 `92689403831f4cb39bd828065a61fab6`, swingo1 `20d55970ff3c412eaad823449fb68e51`, swingo3 `2d9d4295059a408ab329015a3119a260` …, 표는 RtsSkillFxLogic.ShadowClip) — 유닛 자식 "Shadow" 스프라이트, 등 뒤 0.44, OrderInLayer −5(항상 캐릭터 뒤), 본체 액션 이름과 같은 클립을 같은 배속으로. 소환 이펙트 effect `7fab246b200a4dacb4bc9a5aeb7bc0b9`(마법진, 미사용). 풍마수리검은 라이브러리에서 아직 못 찾음(02070008 눈덩이·02070012 종이비행기·02070013 오렌지·02070018 없음).
- Scope: project
- Rationale: 사용자 2026-09-20 — "표창 던지는 모션인데 왜 활이", "표창 생성 위치 배 쪽", "쉐도우 파트너 배우면 등 뒤에", "똑같은 실루엣이면 안 될 것 같은데"(skill/4111002 제시), "캐릭터가 항상 우선", "뇌전수리검으로", "사거리 2".
- Priority: takes precedence over defaults

## 인레이지 레이징 블로우 사운드 = assets/audio/enrage/enrage_sfx2c.mp3 (사용자 확정 2026-09-20)
- Rule: 원작 녹화(bandicam, 셀라스 BGM)에서 추출·후처리한 0.83초 mp3가 최종. 후처리 값: 80Hz 하이패스, 2~4kHz −5.5dB, 6.8kHz 로우패스, 어택 12ms, tanh 리미터, 피크 0.43(−7dB). 사용자는 원본이 "찢어지는 톤·귀 아픔"이라 했고 −8dB/6kHz는 "너무 부드럽다" → 그 사이 값으로 확정. 다른 스킬 사운드를 녹화에서 뽑을 때도 이 범위(2~4kHz −4~−6dB, 로우패스 6.5~7.5kHz, 피크 −6~−7dB)에서 시작. RUID `e794c1382f484195bae36f1b93c9f060`(RtsHeroEnrageSfx, OGG 업로드). 인레이지 사거리는 +1(최대 3).
- Scope: project
- Rationale: 2026-09-20 5회 반복 청취 조정(귀 아픔 → 너무 부드러움 → 중간 → 조금 더 부드럽게 → 확정).
- Priority: takes precedence over defaults

## 섀도어·팬텀 1차 자산(2026-09-20) — 라이브러리에 없는 원작 스킬은 대체품으로
- Rule: 섀도어 새비지 블로우 = 신형 4201012/effect `895ac1e384ea42ca8dd084f54ff3346b`(사용자 지정, 붉은 참격 선) + swingO2 + 타격마다 4221007/hit/0 `6614245a414645089d41ec1f60df1fab` ×8(0.05초) + 타격마다 4201005/hit `23250468023a4767b32611f2fd8ec49b`("8번 파파박"). 메소 익스플로전 = 4211006 effect `61840689ea7b44f09f91960734232e34` + hit/0 `644458b8c91949b4b4b84ff949eba8ca` + hit음 `acb0f70275b7422dbcc8a395cbbd9d28`. 팬텀 조커 = **400041009 본체 + 400041010 타격**(사용자 지정, 3차 2026-09-20): 캐릭터 = **motionCycle { swingOF, swingO1, swingO1, stabO1 } 0.45초 순환**(사용자 확정 "이대로 하자") + keydown 클립 `01eff730ea0a4c3aa797fdb50a1e4492`(리본 회전 잔상)를 loopClip(0.9배, +0.55, α0.9, **loopOrder 190 = 아바타 뒤**, 유닛 자식)으로; 카드 = **사용자 지정 5종 라운드 로빈**(`projs`): 빨강 400041011/screen/6 `e57428fc63e34e239b0ed3c5befdaae3` · 초록 400041012/screen/5 `3642df1a01b043e282621a3d97ee0beb` · 파랑 400041014/screen/4 `4e7909aecedf4dde84d841bf7a0f7fb4` · 검정 400041015/screen/4 `6856aa123ca94eb3ad9315fc7e3bf027` · 노랑 400041013/screen/5 `68eead1e49fa4e21b7ab064d47833a2b` — 원본 ~190×310px지만 보이는 카드는 절반(투명 잔상 테두리)이라 **0.55배**, **회전 없음(projSpin 없음·projNoRotate)·머리 위 projDy 1.35·projArcByDist { 1.0, 1.1, 1.2, 1.3 }(칸 거리 1/2/3/4+ — 항상 위로 볼록, 무작위 없음)**, 2장/공격(0.05초; 공격이 0.1초라 초당 20장), **0.4초 비행·판정 0.35초**(0.65·0.55초 느리게 해 봤다가 사용자 "너무 답답하네, 원래대로" — 카드 비행은 빠른 편이 맞다). (이전 금·갈색 덱 `74e2716e…`, 분홍 덱 `0c3d81a6…`은 안 씀); 타격 hit/0·1·2 `6c691c6d…`·`f0f23e84…`·`8b23673c…`(`hitClips` 무작위) + 타격음 `e37b311a831546b99b6e512f7ba6983c`. 400041009엔 prepare(마법진+카드 흩뿌림)·keydownend·screen(카드 1장→부채→고리) + pre/loop/end 사운드도 있음(미사용). 폐기: 1차 미스틱 카드 24001002/effect 청록 잔상("카드가 안 날아간다"), 2차 24101000 카드 프레임 4장(작고 영상과 다름). 예비 업로드 RtsJokerCardGold `d77f86ea…`/RtsJokerCardPink `73d3cffa…`(prepare/3에서 잘라낸 52×74 카드) 미사용. 5차 스킬(40004xxxx)은 대체로 hit·use 사운드·screen만 있음 — 4차 이하 폴더에서 찾을 것.
- Scope: project
- Rationale: 2026-09-20 도적 3직업 코드 등록(나로 → 섀도어 → 팬텀). 시험대 3번 유닛을 순서대로 교체하며 확인.
- Priority: takes precedence over defaults

## asset_search_resources는 cat에 animationclip을 넣어야 클립이 나온다 — 프레임 sprite와 클립은 별개 항목
- Rule: `asset_search_resources(cat="sprite,audioclip", query="*<스킬id>")`는 개별 프레임 sprite(`…/effect/0`, `…/hit/0/3`)와 아이콘·소리만 돌려준다. 재생용 클립(`…/effect`, `…/hit/0`, resourceType animationclip)은 **`cat="animationclip"`(또는 `sprite,animationclip,audioclip`)**으로 따로 검색해야 한다. query는 경로 세그먼트 단위 일치(`*400041010` OK, `*40004101` 빈 결과). 프레임 sprite 목록은 `projFlySprites`/`frames`/`projHitSprites`에 쓰고, 클립은 `clip`/`hitClip`/`proj`에 쓴다.
- Scope: project
