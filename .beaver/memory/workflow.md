...
- Rule: 게임 버전 표기는 `vYYMMDD-N` — 날짜(연 두 자리·월·일) + 그날 몇 번째 커밋인지(1부터, 날짜가 바뀌면 다시 1). 첫 버전 = **v260924-1**(솔로 출시 — 출시 노트·로드맵 반영 커밋, 이 규칙은 그 커밋부터 센다. 그 전 09-24 커밋 4개는 세지 않음). 커밋 메시지 첫 줄 = `YYYY.MM.DD vYYMMDD-N`(CLAUDE.md Commit). 로드맵 머리말·Progress 줄에 현재 버전을 적는다.
...
- CLAUDE.md application: not needed(작업 방식)
- Priority: takes precedence over defaults
...
- CLAUDE.md application: not needed(디자인 취향)
- Priority: takes precedence over defaults
