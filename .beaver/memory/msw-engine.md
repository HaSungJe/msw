...
## 시전 연출 RPC는 FaceDir 동기화보다 먼저 올 수 있다 — 방향은 RPC에 실어 보낸다
- Rule: 유닛 시전 연출은 `RtsCombatLogic.CastFxFace(zone, no, idx, names, face)`로 방향을 같이 보내고, 클라는 `RtsUnitComponent.SetLocalFace` → `FaceNow()`(LocalFace 우선)로 아바타·이펙트를 같은 방향에 맞춘다. 서버는 몸 동작 중(`FaceLockUntil`, MotionSec 최대 1.2초) 방향을 안 바꾼다.
- **Why:** 2026-09-23 사용자 "스킬은 왼쪽으로 나가는데 캐릭터가 오른쪽으로 고개를 돌리며 모션" — RPC가 먼저 와서 이펙트는 옛 방향, 아바타는 동기화 때 뒤늦게 돌았다.
- **How to apply:** 방향이 들어가는 새 연출은 `FacingRight`/`FaceNow`를 쓰고 `u.FaceDir`를 직접 읽지 않는다.

## 타격 판정은 판정 순간 대상을 다시 확정한다
