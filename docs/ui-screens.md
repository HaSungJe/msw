...
| 아이콘 칸 | `RtsProfileIconSlot` | 〃 | 줄 안 a(0,0.5) p(0,0.5) (10,0) 48×48, `MaskComponent` | 이미지 = `CircleMaskRUID`(원형), (0.16,0.14,0.12) — 아이콘 없음이면 빈 원에 α0.45, 글자 없음 | 항상 | — |
| 아이콘 | `RtsPortrait` | `SpawnMonsterPortrait` | 칸 가운데 48, 멈춘 그림 | 원색 | 내 아이콘이 있을 때. `RefreshProfileBox`가 아이콘이 바뀐 때만 다시 만든다 | — |
...
| 아이콘 그림자 | `RtsPlayerIconShadow` | 〃 | 카드 사진 영역 가운데 a/p(0.5,0.5), (0,17) 88×88 | 원형 약한 그림자 α0.28, 클릭 통과 | 아이콘이 있을 때만 | — |
| 아이콘 칸 | `RtsPlayerIconSlot` | 〃 | 카드 사진 영역 가운데 a/p(0.5,0.5), (0,20) 80×80, `MaskComponent` | 원형 밝은 베이지 α0.72. 아이콘 없음이면 칸 자체를 만들지 않아 배경 풍경이 보임 | 아이콘이 있을 때만 | — |
| 아이콘 | `RtsPortrait` | `SpawnMonsterPortrait` | 칸 가운데 80, 멈춘 그림(표 `icon`) | 원색, 탈락이면 회색 | 아이콘이 있을 때만 | — |
...
| `iconinfo` | 아이콘 | 640×370 | 아이콘 칸 클릭 · 확인에서 [아니오] | 초상 140(보유면 움직임) + 이름 + 상태(사용 중 / 보유 · 획득일 / 미획득) + 획득처(`WhereText`: 매우어려움 · n라운드 · 지역 - 맵) + 확률 + [이 아이콘 사용](보유·미사용) · [목록으로](→ `profile` [아이콘] 탭) |
