...
| 10~13 | 보조 라벨(13), 슬롯 Lv·키캡·잠금 토글(12), 스킨 안내(11), 디버프 남은 초(10) |
...
| Q | 순위 `RtsRankBtn` | 16~146, 890~930 | RtsHudLogic `RtsRankBtn` |
...
| 순위 (2026-09-24) | `RtsRankBtn`/`In`, `RtsRankLabel`, `RtsRankClick` | 도감 버튼 바로 아래 | a(0,0) p(0,0) (16,150) 130×40, 테두리 1.5. 글자 126×34 17 굵게 | ColPanel/ColBorder | 항상 | `_RtsPopupLogic:Open("rank")` |
...
| 초상 칸 | `RtsMonInfoIconBack` | 104-108 | a(0,0.5) p(0,0.5) (12,0) 94×94 | (0.2,0.17,0.15). **MaskComponent**로 칸 밖을 자른다 | — |
| 초상 | `RtsMonInfoIcon` | 111-125 | 칸 가운데 86×86. 걷기 클립 반복, NativeSize, LocalScale = 칸에 맞춘 배율(최대 2.5) | 원색 | 크기·위치는 ImageRUID를 넣은 **뒤에** 준다(:123) |
| 이름 | `RtsMonInfoName` | 127 | a(0,1) p(0,1) (118,-8) 440×28 | ColInk 19 굵게 리치. 보스면 뒤에 `<color=#e0b95a>보스</color>` | — |
| 체력바 | `RtsMonInfoHpBack` / `RtsMonInfoHpFill` | 131-134 | (118,-42) 366×26. 채움 폭 = 366 × 비율 (:186) | (0.2,0.17,0.15) / (0.85,0.2,0.2) | — |
| 체력 글자 | `RtsMonInfoHpText` | 135 | 체력바와 같은 사각형 | 흰색 15 굵게 가운데 "현재 / 최대" | — |
| 체력 % | `RtsMonInfoHpPct` | 137 | (488,-42) 70×26 | ColInk 16 굵게 오른쪽 "%.1f%" | — |
| 정보 줄 | `RtsMonInfoText` | 139 | (118,-76) 440×28 | ColMuted 15 | "방어율 n% (보스만) · n라운드 · 맵 이름" |
| 디버프 줄 | `RtsMonInfoDebuffs` | 280 | 패널 바깥 기준 a(0,1) p(0,0) (118,6), 아이콘 수×42 × 36 | 투명 | 디버프 목록이 바뀔 때만 다시 만든다 |
| 디버프 아이콘 | `RtsMonInfoDebuff{id}` | 285-306 | 36×36, 6 간격 | RUID가 있으면 그림(가드 크러쉬 `b28058e3…`, 방어구 부수기 `1f4eea78…`). 없으면 색 배지 + 글자 17(빙 (0.45,0.78,1) · 파 (0.3,0.55,0.95) · 약 (0.95,0.8,0.35) · 독 (0.45,0.8,0.35)). 아래 36×13 검은 띠(α0.55)에 남은 초(10 굵게) | `UITouchReceiveComponent` + UITouchEnter/Exit로 말풍선 |
| 말풍선 | `RtsMonInfoTip`/`In` + `Text` | 344-348 | 디버프 줄 기준 a(0,1) p(0,0) (x,8) 300×82, 글자 280×72 | (0.11,0.094,0.075,0.97)/ColGold, ColInk 14 리치 | 이름 · 설명 · 남은 시간 |
...
| `restart` | 다시하기 | 640×280 | 오른쪽 아래 다시하기 · 결과 창 다시하기 · 판이 끝난 뒤 오른쪽 버튼(`OpenRestartConfirm`, 2026-09-24) | 안내 세 줄 + [예 · 다시하기](솔로) / [예 · 찬성](여럿) + [아니오]. 이미 찬성했으면 다시하기 버튼 = 바로 찬성 취소 |
| `rank` | 순위 | 720×820 | HUD 순위 버튼 | 난이도 탭 3(어려움·매우어려움·극악, `RankDiff`) + 안내 줄 + 목록(스크롤, 1~3위 금색: 순위·닉네임·클리어 n회) + 아래 내 순위 띠. 목록은 서버 `RequestRanking` → `ShowRanking` → `SetRanking`(`RankCache`, 20초 캐시) |
...
| 증강 합계 | `RtsUnitAugSum` | 655 | 목록 맨 위 (rw−12)×88 | Mix(Gold,0.08)/Mix(Gold,0.4). 3줄(Gold 14 / Ink 15 / Muted 13) |
...
- 클리어면 `RtsRsFinal`(−160, Gold 20 "<난이도> 검은 마법사 클리어 n회째")과 `RtsRsRank`(−194, 18 "<난이도> 순위 m위 · 전체 순위는 왼쪽 아래 '순위'")가 더 붙는다(`ClearRank` ← `SetClearRank`). 보통 이하면 "순위는 어려움 · 매우어려움 · 극악 클리어만 기록해요", 테스트 모드면 "테스트 모드 — 순위에 기록하지 않아요".
- 버튼(y −186, 클리어면 −250), 둘 다 180×44: `RtsRsClose`(SpawnTab, 가운데 왼쪽)는 닫기만 한다 — 솔로 "닫기" / 여럿 "관전하기". `RtsRsExit`(가운데 오른쪽)는 솔로면 "다시 하기"(금색 — `RtsStageLogic.RequestRestartVote`, 혼자 찬성 = 바로 새 판), 여럿이면 "나가기"(위험색 — `RequestExit`). 솔로 판정 = `RtsStageLogic:IsSolo()`(방 인원 1).
...
- 결과 팝업의 "최종 메소·클리어 순위" 두 줄은 옛 순위 기준이다(8장 #44 — 솔로 출시 Phase 12에서 교체).

---

## 8. 아직 없는 / 예정 UI (로드맵 `.beaver/output/roadmap/maple-augment-defense-roadmap.md`)

출시 노트(v260924-1) 개발 예정 순서: ① 멀티플레이(#31 대기방·매칭·방) → ② 순위표(#42 대기방 순위표) → ③ 개인 테마 프리셋(#38) → ④ 아이콘 프리셋(#40) → ⑤ 전체 UI 개선(#41 플레이어 카드·#36 디자인 패스).
...
| 44 | 순위 기준 교체 + 결과 창 순위 (**v260924-1에 구현됨** — 4.2 `rank` 창·결과 창 참고) | 순위 = **어려움·매우어려움·극악에서 검은 마법사 4페이즈를 깬 횟수**. 클리어할 때마다 서버가 +1한다(테스트 모드 제외). 결과 창에 내 순위와 횟수를 보여 준다 | 세 난이도 합산 1개 표 vs 난이도별 탭 3개, 동점 2차 기준 |
