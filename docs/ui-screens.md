...
| 정보 | (128,-102), 494×46 | 보스 방어율 + 줄바꿈 후 라운드·맵, 보조색15px |
...
보유 메소, 네 등급의 다음 뽑기 확률, 프리즘 남은 횟수와 천장, 선택 대기 수를 표시한다. 확률은 `BuyPrismRate`와 서버의 일반 등급 비율로 계산한다. 뽑기 버튼470×76은 가격·메소 부족·한도·일시정지 상태를 표시하고 기존 `BuyFromHud/RequestBuy`로 요청한다. 값이 바뀌면 해당 라벨만 갱신한다.
...
통합 뽑기 탭에 같은 크기의340×660 카드 세 장을 표시한다. 등급 띠·104px 합성 아이콘, 이름21px, 효과18px(302×364), 받기302×44. 지속 반짝임 타이머는 제거하고 정적인 등급색 가장자리·등장음을 사용한다.

받기는 기존 수령 요청 후 보유 탭으로 이동한다. 일시정지 상태에서는 선택을 닫지 않고 안내한다. 보유/도감으로 이동하거나 ×를 눌러도 선택지는 남으며 통합 뽑기 탭에서 이어 고른다. 최대 대기 수는 `MaxPending` 값을 사용한다.
...
- 3택1 카드: 등급(y-12)·아이콘(y-42)·이름(y-158)·효과(y-212)가 같은 가로 중앙 축이다. 효과는 UpperCenter로 시작 높이를 맞추며 받기 버튼과 분리한다.
...
- 증강 선택 카드 중앙 배치: 카드 340×660, 등급은 상단·받기는 하단 유지. 아이콘104 + 간격20 + 제목64 + 간격12 + 설명 높이를 한 묶음으로 중앙 배치. 설명 높이는 문장 길이에 따라 52~230, 제목과 설명은 MiddleCenter.
