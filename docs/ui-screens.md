...
| kind | 제목 | 크기 | 여는 곳 | 구성 |
|---|---|---|---|---|
| `recruit` | 캐릭터 영입 | 1114×760 | HUD 영입 버튼(RtsHudLogic:707). 판 시작 첫 틱에 자동으로 한 번(RtsStageLogic:444-448). 칸 수가 바뀌면 다시 그림(RtsHudLogic:759) | 탭 2 + 계열 4열 × 직업 카드 + 하단 안내. **카드는 모두 클릭 → `PendingJob` → `recruitinfo`**(2026-09-25 — 보유 중·잠김·영입 불가도 보기) |
...
| 영역 | 구현·배치 | 동작 |
|---|---|---|
| 흉상 | `RtsUnitLook` (27,-10) 190×416. 썬콜180×180, 가운데 (95,-126) | 승인된 `portrait.png`를 재사용. 다른 직업은 기존 아바타 |
...
모험가/영웅 탭 아래 계열 4열, 열 폭250·간격14. 카드250×150, 행 간격162. 썬콜은 승인된 흉상96×96, 다른 직업은 기존 아바타72×86. 그림 아래 공식 직업명19px·상태15px. 보유·잠김 상태도 상세 열기는 가능하며 영입 검증은 기존 로직이다. 영입 상세도 같은 흉상을 사용한다.
