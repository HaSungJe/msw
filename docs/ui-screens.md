...
(위 ASCII 그림의 A·B 자리는 2026-09-25 전 기준 — 지금은 T(테마 상자) 아래로 A·B가 72px씩 내려갔다. 좌표는 이 표가 맞다.)
...
| 요소 | 엔티티 이름 | 만드는 곳 | anchor·pivot·위치·크기 | 색·글꼴 | 보이는 조건 | 클릭/동작 |
|---|---|---|---|---|---|---|
| 카드 틀 | `RtsInfoCard`/`In` | BuildHud | a(0,1) p(0,1) (16,-88) 440×138, 테두리 1.5 (2026-09-25 테마 상자 아래로) | ColPanel / ColBorder | 항상 | — |
...
### 3.2 좌측 유저 카드 (B) — 2026-09-25 (사용자 "영역을 키워서 배경 = 그 사람 테마 썸네일, 좌측 동그라미 아이콘, 우측 닉네임, 나는 (나) 대신 굵은 초록 테두리")
| 요소 | 엔티티 이름 | 만드는 곳 | anchor·pivot·위치·크기 | 색·글꼴 | 보이는 조건 | 클릭/동작 |
|---|---|---|---|---|---|---|
| 목록 틀 | `RtsPlayers` | BuildHud | a(0,1) p(0,1) (16,-236) 280×520 | 투명 | 항상 | — |
| 머리 "n / 8" | `RtsPlayerHead` | 〃 | a(0,1) p(0,1) (6,0) 200×20 | (0.49,0.525,0.584) 13 | 항상 | 분모는 구역 수 |
| 카드 i | `RtsPlayerRow{i}`/`In` | `AddPlayerCard` | a(0,1) p(0,1) (0, −24−62·(i−1)) `CardW`280×`CardH`56. 테두리 = 나 3px 초록(0.35,0.8,0.42) / 남 1.5 | 안쪽 = 그 유저 테마 `thumb`(없으면 테마 `color`). 테두리: 나 초록 · 보는 중 ColGold · 그 외 ColBorderSoft | 서버 통지마다 전부 지우고 다시 만든다 | 클릭 → `OnPlayerRowClick` → 그 구역으로 카메라 |
| 어두운 막 | `RtsPlayerShade` | 〃 | 카드 전체 | 생존 (0.06,0.05,0.04,0.35) / 탈락 α0.68 | 〃 | — |
| 아이콘 칸 | `RtsPlayerIconSlot` | 〃 | a(0,0.5) p(0,0.5) (6,0) 44×44, `MaskComponent` | 이미지 = `CircleMaskRUID`(원형 마스크, 없으면 네모), (0.16,0.14,0.12) | 〃 | — |
| 아이콘 | `RtsPortrait` | `SpawnMonsterPortrait` | 칸 가운데 44, 멈춘 그림(표 `icon`) | 원색, 탈락이면 회색 | 〃 | — |
| 닉네임 | `RtsPlayerName` | 〃 | a(0,0.5) p(0,0.5) (54,0) 폭 = 280−44−20(나 −50) × 30 | ColInk 17 굵게(탈락 (0.604,0.561,0.502)) | 〃 | — |
| 비석 | `RtsPlayerTomb`/`In` | 〃 | a(1,0) p(1,0) (-8,6) 14×17 | (0.29,0.27,0.235) / (0.55,0.51,0.45) | 탈락 | — |
| [수정] | `RtsPlayerEdit`/`In` + `Text` | 〃 | a(1,1) p(1,1) (-6,-6) 44×22 | 배경 (0.11,0.094,0.075,0.9) / ColGold 1, 글자 ColGold 12 | **내 카드만** | → `_RtsPopupLogic:Open("icon")` |
...
| `theme` | 맵 테마 | 900×660 | 테마 상자(T) 클릭 · 확인에서 [아니오] | 탭 줄(`GetTabs` — 지금 '기본') + 안내 줄 + 테마 카드 3열(270×220: 썸네일 250×150 `PaintThumb` + 이름 + '사용 중'/클릭). 카드 → `PendingTheme` → `themeconfirm` (2026-09-25) |
| `themeconfirm` | 맵 테마 | 640×300 | 테마 카드 클릭 | "맵 테마를 변경하시겠습니까? → 이름" + [예](`RequestSetTheme`) / [아니오](→ `theme`) — `BuildConfirm` 공용 |
| `icon` | 아이콘 | 900×700 | 내 유저 카드 [수정] · 정보에서 [목록으로] | 탭 줄('기본') + "보유 n / 247" 줄 + 한 쪽 32칸(8×4, 칸 94, 간격 8 — `IconPerPage`, 247칸 한꺼번에 만들면 UI 1,000개+라 느림) + ‹ 이전 / n / 8 / 다음 ›. 보유 원색 · 미보유 실루엣 · 사용 중 금색 칸. 칸 → `PendingIcon` → `iconinfo` (2026-09-25) |
...
| # | 화면 | 로드맵에 적힌 설계 | 미정 |
|---|---|---|---|
| 41 | 좌측 유저 카드 | **2026-09-25 뼈대 구현됨 — 3.2 참고**(배경 = 테마 썸네일, 원형 아이콘(`CircleMaskRUID` 69e2dced…), 닉네임, 내 카드 초록 테두리 + [수정]) | 카드 디자인(ChatGPT) |
...
| 38 | 맵 테마 | **2026-09-25 뼈대 구현됨 — 3.0 테마 상자 · 4.2 `theme`·`themeconfirm` 참고**. 헤네시스·엘리니아·페리온·커닝시티·리스항구 5종 무료, 내 구역만. 헤네시스 외 4종은 임시 모양 | 실제 디자인(바닥·트랙·장식·썸네일) — `docs/theme-presets.md` |
