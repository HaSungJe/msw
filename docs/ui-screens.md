...
| `result` | 결과 | 640×360 (클리어면 430) | 서버 `ShowResult` → `OpenResult`(:206). 탈락·클리어한 본인에게만 | 머리글 + 줄 4개 (+ 클리어 2줄) + [관전하기][나가기] |
...
- 클리어면 `RtsRsFinal`(−160, Gold 20 "최종 메소 …")과 `RtsRsRank`(−194, 18 "클리어 순위 …")가 더 붙는다. **이 두 줄은 옛 순위 기준(최종 메소)이며 로드맵 #44에서 바뀔 예정이다.**
- 버튼(y −186, 클리어면 −250), 둘 다 180×44: `RtsRsClose` "관전하기"(SpawnTab, 가운데 왼쪽)는 닫기만 한다. `RtsRsExit` "나가기"(가운데 오른쪽, 위험색)는 `RequestExit`(Server)를 부른다.
...
| 다시 하기(D) | 흐림(idle 모드), 눌러도 무시 | 〃 | 누를 수 있음(3초 확인) | 누르면 바로 찬성 |
| 투표 패널(E) | 표가 있을 때만 | 〃 | 〃 | 〃 |
| 영입 버튼(R) | 숨김 | 숨김 | 빈 칸이 있을 때만 "영입 가능 n" | "나가기" |
...
- 영입 버튼 자리가 "나가기"(위험색)로 바뀐다(RtsHudLogic:722-736).
