...
| 스킨 버튼 2 | `RtsUnitSkin1/2` | 255 | (146, −34−64·(i−1)) 205×58 | 선택: Mix(Gold,0.18)/Gold. 잠김: 테두리 Mix(Gold,0.55)에 "1,600 월드코인 · 30일 해금", 클릭하면 월드 상점(`RequestBuyAvatar`) |
...
| 44 | 순위 기준 교체 + 결과 창 순위 | 순위 = **어려움·매우어려움·극악에서 검은 마법사 4페이즈를 깬 횟수**. 클리어할 때마다 서버가 +1한다(테스트 모드 제외). 결과 창에 내 순위와 횟수를 보여 준다 | 세 난이도 합산 1개 표 vs 난이도별 탭 3개, 동점 2차 기준 |
...
| 39 | 아바타 프리셋(후순위) | 월드 아바타 30일권을 영구 이용권으로 바꾸고, 직업별 아바타 프리셋을 유닛 팝업 외형 영역에서 고른다 | 슬롯 수, 저장 단위 |
