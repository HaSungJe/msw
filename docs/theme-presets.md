...
- 화면: 좌상단 **프로필 상자**(썸네일 + "프로필 설정" + "테마 ○○ · 비석 ○○", 누르면 프로필 창) · **프로필 창 [테마] 탭**(카드) · **유저 카드 배경**(그 유저 테마 썸네일). 같은 창 [비석] 탭 = 비석 프리셋(6절).
...
| `thumb` | 썸네일 스프라이트 RUID(`""` = 없음) | 됨 — 권장 400×240(5:3). 프로필 창 카드 250×200(정사각 카드 270 안) · 프로필 상자 104×48 · 유저 카드 배경 280×56에 같이 쓰임 |
...
|---|---|---|---|---|---|
| `henesys` | 헤네시스 | HenesysGrass1 + 2·3 (12%) | `b1a2d2e1…`(ZoneTrackGridM2 16×13) 흰색 | 헤네시스 탑뷰 23개 | 없음(초록) |
...
2. **트랙 판** — `tools/gen-track-grid.js`(16×13, 칸 80px, 1280×1040 — 2026-09-25 M2 경로 77칸·매듭 3곳·보스 영역 가운데 2×2는 비움, S = 진행 방향 화살표 · E = 되돌리기 아이콘)의 포석·연석·발판·선 색을 바꿔 생성 → 업로드 → `gridRUID`, `gridTint`는 흰색. **경로(SEQ)·칸 수는 건드리지 않는다**(바꾸면 `RtsZoneLogic.GetTurnPoints`·`RtsConfigLogic`까지 바뀌어야 함).
...
| `ruid` · `scale` | 월드 비석 스프라이트 · 월드 스케일(기본 5) | 됨 |
| `w` · `h` | 원본 그림 px — 프로필 창 카드(250×190 안)에 비율 맞춰 넣을 때 | 그림을 바꾸면 같이 |
| `free` | `true` = 누구나 보유. `false`면 보유 목록(`"tombs"`)에 있어야 고를 수 있다 | 됨 |

지금 1종: `basic` 기본 비석 = 원작 묘비(map/obj/jpsengoku2014/east_out/gravestone) `29e864c6…` 148×239.
